...
## Understanding the topics through visualisation
An effective way to understand the topics is by evaluating the term distributions associated with them. The LDA Gensim library provides a well-designed, interactive visualisation tool - LDA.vis - to help users assess the quality of individual topics and of all topics as a whole as well as facilitate interpretability. The primary visualization used in Termite is a matrix view; rows correspond to terms and
columns to topics
We define term saliency as follows. For a given word w,
we compute its conditional probability P(T|w): the likelihood that observed word w was generated by latent topic T.
We also compute the marginal probability P(T): the likelihood that any randomly-selected word w
0 was generated
...
sense) how informative the specific term w is for determining the generating topic, versus a randomly-selected term
w
0
. For example, if a word w occurs in all topics, observing
...
same number of words, the list of most probable terms contains more generic words (e.g., based, paper, approach) than
...
We experimented with trigram statistics, but find that
it degrades the overall seriation quality
yout algorithm, large data set, and
...
Paper 2 (Sievert & Shirley 2014)
LDAvis allows one to select a topic to reveal the most relevant terms for
that topic. 
30 most relevant terms (given  = 0.34, in this
...
If  = 1, terms are ranked solely by kw, which
implies the red bars would be sorted from widest
(at the top) to narrowest (at the bottom). By comparing the widths of the red and gray bars for a
given term, users can quickly understand whether
a term is highly relevant to the selected topic because of its lift (a high ratio of red to gray), or
its probability (absolute width of red). its corpus-wide occurrences, whereas “cruel” is a
relatively rare term with very high lift in Topic 34
– it occurs almost exclusively in this topic. Such
properties of the topic-term relationships are readily visible in LDAvis for every topic.
On the left panel, two visual features provide
a global perspective of the topics. First, the areas of the circles are proportional to the relative
...
6% of the corpus, and all contain common, nonspecific terms (although there are interesting differences: Topic 2 contains formal debate-related
language such as “conclusion”, “evidence”, and
“argument”, whereas Topic 3 contains slang conversational language such as “kinda”, “like”, and
“yeah”). In addition to visualizing topic prevalence, the left pane shows inter-topic differences.
The second core feature of LDAvis is the ability to select a term (by hovering over it) to reveal
its conditional distribution over topics. This distribution is visualized by altering the areas of the
topic circles such that they are proportional to the
term-specific frequencies across the corpus. 
 In addition, there is an option to apply k-means clustering to the topics (as a function of their two-dimensional locations in the global
...
the cluster. relevance, by which
