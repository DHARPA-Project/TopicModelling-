...
An effective way to understand the topics is by evaluating the term distributions associated with them. The LDA Gensim library provides a well-designed, interactive visualisation tool - LDA.vis - to help users assess the quality of individual topics and of all topics as a whole as well as facilitate interpretability. In order to interpret the topics, the first thing to consider is that each topic is a combination of keywords and that each keyword contributes a certain 'weight' to the topic. Understanding the weight of each keyword is crucial to understand how important that keyword is to that topic. LDA.vis provides this information by visualising each keyword's *saliency* (Chuang et. al (2012), defined as the relationship between the likelihood that observed word w was generated by latent topic T and the likelihood that any randomly-selected word w' was generated by topic T. In other words, the saliency represents how informative the specific term w is for interpreting the topic, versus a randomly-selected term w'. The way saliency is visualised is by showing each keyword's overall frequency in comparison with the estimated term frequency within the selected topic. Filtering terms by saliency is helpful for a rapid classification and disambiguation of the topics (ibid.) as well as for the identification of non-informative topics lacking salient terms.

LDA.vis also allows for a general visualisation of the topics (i.e., the larger the areas of the circles, the higher the weight of the topic in the corpus) and their distribution across the corpus (i.e., inter-topic differences) (Sievert & Shirley 2014). It is also possible to visualise the conditional distribution of a select a term (by hovering over it) over topics (i.e., which topics share the same keywords). In addition, there is an option to apply k-means clustering to the topics (as a function of their two-dimensional locations in the global
