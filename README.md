...
Model perplexity and topic coherence are two ways to measure the statistical quality of a topic model. **Model perplexity** (also known as predictive likelihood) is a way to predicts the likelihood of new (unseen) text to appear based on a a pre-trained model. The lower the value, the better the model has been designed to predict the sample, in this case, the words that appear in each topic. However, studies have shown that optimizing a language model for perplexity does not increase interpretability, as perplexity and human judgment are often not correlated, and sometimes even slightly anti-correlated (Jacobi et al., 2015, p. 7). **Topic Coherence** on the contrary seems to have become more popular in TM for assessing the so called 'optimal' number of topics.    
