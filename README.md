...
When working with historical collections of timestamped documents, the topics' distribution can be plotted over time, thus becoming particularly relevant to humanities scholars that try to answer historical questions. This operation, while essential to historicise the topics, is however dependant on a nu,ber of factors. There are both technical (e.g., standard modelling tools such as MALLET or Gensim do not provide this functionality, the nature of the data) and theoretical reasons (e.g., whether time is understood as continuous or discreet) for why that is. Thus, depending on these determining factors, there are different strategies to achieve this goal.
